...
function Navbar({ cart, addCart, removeCart, clearCart, subTotal }) {
    ...
    return (
        <div className='flex flex-col md:flex-row md:justify-start justify-center items-center px-5 py-2 md:py-0 bg-white sticky top-0 z-50 shadow-md'>
            ...
            <div ref={ref} className="sidebar absolute top-0 right-0 bg-gray-100 p-10 transition transform translate-x-full ease-in-out duration-300">
                ...
                <ul className='space-y-3'>
                    {
                        Object.keys(cart).map((item) => (
                            <li className='space-x-10 flex items-center justify-center'>
...

                {
                    Object.keys(cart).length >= 1 ? (
                        <div className='flex'>
                            <button className="flex mx-auto mt-16 text-black border-2 border-black bg-transparent py-2 px-4 focus:outline-none hover:bg-gray-200 rounded text-base"><AiOutlineShoppingCart className='mt-1 mr-2 text-xl' />Button</button>

                            <button onClick={clearCart} className="flex mx-auto mt-16 text-black border-2 border-black bg-transparent py-2 px-4 focus:outline-none hover:bg-gray-200 rounded text-base">Clear</button>
                        </div>
                    ) : (
